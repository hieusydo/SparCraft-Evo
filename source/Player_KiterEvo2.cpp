...
Player_KiterEvo2::Player_KiterEvo2(const IDType & playerID)
{
	_playerID = playerID;
	_Wup.init(0);
	_Wdown.init(0);
...
Player_KiterEvo2::Player_KiterEvo2(const IDType & playerID, const std::string& filePath) : _wFile(filePath) {
	_playerID = playerID;
	_Wup.init(0);
	_Wdown.init(0);
	_Wleft.init(0);
...
void Player_KiterEvo2::switchOnOffline() {
	_offline = true;
}

void Player_KiterEvo2::switchOffOffline() {
	_offline = false;
}

// Order of direction Array in vector: Left Right Up Down
void Player_KiterEvo2::setWeights(const std::vector<Array<double, Constants::Num_Params>>& weights) {
...
Dxy Player_KiterEvo2::getDxyClosest(const Unit& closestUnit, const Unit& ourUnit) const {
	Dxy res;
...
Dxy Player_KiterEvo2::getDxyCenter(const Position& centerArmy, const Unit& ourUnit) const {
	Dxy res;
...
size_t Player_KiterEvo2::getMaxVDir(double allV[4]) const {
	double maxV = -DBL_MAX;
	size_t maxIndex = 4;
	for (size_t i = 0; i < 4; ++i) {
		if (allV[i] > maxV) {
...
void Player_KiterEvo2::normalize(Array<double, Constants::Num_Params>& X) {
	double meanX = 0;
	double minX = DBL_MAX;
	double maxX = -DBL_MAX;
...
void Player_KiterEvo2::printWeights() const {
	std::cout << "\nPrinting...\n";
	std::cout << "_Ws:" << _Wleft << "\n" << _Wright << "\n" << _Wup << "\n" << _Wdown << "\n";
}

void Player_KiterEvo2::getMoves(GameState & state, const MoveArray & moves, std::vector<Action> & moveVec)
{
	moveVec.clear();
