...
void SearchExperiment::bruteForceBestDist() {
	...
	std::ofstream atkClosestDat;
	atkClosestDat.open("atkClosestDat.csv");
	atkClosestDat << "d,score\n";
	for (size_t i = 1; i < 500; i++) {
		p1Evo->switchOnOffline();
		p1Evo->setSafeDist(i);
		Game g(states[0], p1, p2, 1000);
		g.play();
		int s = g.getState().evalLTD2(Players::Player_One);
		if (s > maxScore) {
			maxScore = s;
			bestDist = i;
		}
		//std::cout << "d= " << i << " - score = " << s << "; ";
		atkClosestDat << i << "," << s << "\n";
	}
	atkClosestDat.close();
	std::cout << "Best dist found by brute force: " << bestDist << " with score = " << maxScore << "\n";
}
